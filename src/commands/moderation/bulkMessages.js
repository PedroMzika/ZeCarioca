...
module.exports = class AvatarCommand extends Command {
	constructor(client) {
 	...
 	async run ({ message, channel }, args) {
		...
		if (numberMessages <= 0) return channel.send(new ParrotEmbed() .setDescription("⚠️ | Insira um número positivo!"));

		channel.messages.fetch({ limit: numberMessages }).then(messages => {
			message.channel.bulkDelete(messages);
		});

		channel.send(new ParrotEmbed() .setDescription(`🥳 | Eu consegui apagar ${numberMessages} ${numberMessages > 1 ? "mensagens" : "mensagem"}!`)).then(msg => msg.delete({ timeout: 30000 }));
	}  
};
