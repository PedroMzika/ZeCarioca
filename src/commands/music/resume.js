...
module.exports = class ResumeCommand extends Command {
  ...
  async run ({ message, author, channel, member }) {
    ...
    if (player.paused) {
      message.channel.send(new ParrotEmbed(author).setDescription('<:musicPlay:708136949755674654> | A música foi retomada.')).then(msg => msg.delete({ timeout: 15000 }))
      player.pause(false)
    } else {
      message.channel.send(new ParrotEmbed(author).setDescription('⚠️ | Á música não está pausada.')).then(msg => msg.delete({ timeout: 15000 }))
    }
  }
}
