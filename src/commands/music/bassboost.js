...
module.exports = class BassBoostCommand extends Command {
  ...
  async run ({ message, author, channel, member }) {
    ...
    if (!player || player.queue.length <= 0) return channel.send(new ParrotEmbed(author).setDescription('⚠️ | Não há nenhum player tocando no momento.')).then(msg => msg.delete({ timeout: 30000 }))
    ...
    channel.send(new ParrotEmbed(author).setDescription(`<:musicSettings:708136949487239198> | O Bassboost foi definido como ${enabled ? '`ligado`' : '`desligado`'}!`)).then(msg => msg.delete({ timeout: 30000 }))
