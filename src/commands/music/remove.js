...
module.exports = class RemoveCommand extends Command {
  ...
  async run ({ message, author, channel, member }, args) {
    ...
    if (author.id === player.track.info.requester.id || author.id === player.dj.id) {
      if (isNaN(args[0])) return channel.send(new ParrotEmbed(author).setDescription('⚠️ | Coloque um **número** da música que quer pular!')).then(msg => msg.delete({ timeout: 15000 }))
      if (!args[0]) return channel.send(new ParrotEmbed(author).setDescription('⚠️ | Coloque o **número** da música que quer pular!')).then(msg => msg.delete({ timeout: 15000 }))
      if (args[0] > player.queue.length) return channel.send(new ParrotEmbed(author).setDescription('⚠️ | Não há essa quantidade de música na playlist!')).then(msg => msg.delete({ timeout: 15000 }))

      if (args[0] === 0) player.stop()

      player.queue.remove(args[0])
      channel.send(new ParrotEmbed(author).setDescription('<:musicFast:708136949143175239> | Música removida com sucesso!')).then(msg => msg.delete({ timeout: 15000 }))
    } else {
