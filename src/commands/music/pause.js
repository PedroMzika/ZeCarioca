...
module.exports = class PauseCommand extends Command {
  ...
  async run ({ message, author, channel, member }) {
    ...
    if (!player.paused) {
      message.channel.send(new ParrotEmbed(author).setDescription('<:musicPause:708136948966883350> | A música foi pausada.')).then(msg => msg.delete({ timeout: 30000 }))
      player.pause(true)
    } else {
      message.channel.send(new ParrotEmbed(author).setDescription('⚠️ | Á música já está pausada.')).then(msg => msg.delete({ timeout: 30000 }))
    }
  }
}
